...
Процедура ДобавитьФайл()
    ...
    Иначе
        //кп лена 
        //добавлено оповещение чтобы призходила рассылка в телегу
		//РаботаСФайламиКлиент.ДобавитьФайлы(Параметры.ВладелецФайла, УникальныйИдентификатор, , ГруппаФайлов);
		РаботаСФайламиКлиент.ДобавитьФайлы(Параметры.ВладелецФайла, УникальныйИдентификатор, , ГруппаФайлов, новый ОписаниеОповещения("КП_ОповеститьОДобавленииФайла",ЭтаФорма));
	КонецЕсли;
...
//добавлено оповещение чтобы призходила рассылка в телегу
&НаКлиенте
Процедура КП_ОповеститьОДобавленииФайла(массивФайлов, ДП) Экспорт//       
    
    КП_ОповеститьОДобавленииФайлаСервер(массивФайлов, ДП);
    
КонецПроцедуры

&НаСервере
Процедура КП_ОповеститьОДобавленииФайлаСервер(массивФайлов, ДП)      
    
    для Каждого стр из массивФайлов цикл
        Если стр.ВладелецФайла.Техподдержка тогда Продолжить; КонецЕсли;
        
        Текст = Символы.ПС + "*Добавлено вложение:* `" + стр.Наименование + "." + стр.Расширение + "`"; 
        Текст =  "*Изменения в заявке :*  " + Формат(число(стр.ВладелецФайла.Номер), "ЧДЦ=0; ЧГ=0") + 
        Символы.ПС + "*Контр-т*:  " + стр.ВладелецФайла.Контрагент + 
        Символы.ПС + "*Описание:*  " + стр.ВладелецФайла.Название + Текст; 
        
        мПараметры = Новый Массив;
        мПараметры.Добавить(стр.ВладелецФайла.Смета.ВыгрузитьКолонку("Сотрудник"));
        мПараметры.Добавить(Текст);
        мПараметры.Добавить(ПараметрыСеанса.ТекущийПользователь);
        мПараметры.Добавить(стр.ВладелецФайла.Ссылка);
