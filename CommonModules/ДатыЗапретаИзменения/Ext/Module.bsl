...
Функция ИзменениеЗапрещено(ДанныеИлиПолноеИмя, ИдентификаторДанных = Неопределено,
    ...
    //+кп никитина 23.03.20
    Если ТипЗнч(ИдентификаторДанных) = Тип("ДокументСсылка._Заявка") тогда
        Если  ИдентификаторДанных.Статус = Перечисления._СтатусыЗаявки.ВРаботе или 
            ИдентификаторДанных.Статус = Перечисления._СтатусыЗаявки.Принят 
            или ИдентификаторДанных.Статус = Перечисления._СтатусыЗаявки.ПереданоВТестирование  тогда
            возврат Ложь
        КонецЕсли;
    КонецЕсли;
    //-кп никитина 23.03.20
...
Процедура ПроверитьДатуЗапретаИзмененияПередЗаписьюДокумента(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
    ...
    //+кп никитина 23.03.20
    Если  ТипЗнч(Источник.ссылка) = Тип("ДокументСсылка._Заявка") тогда
        Если не  (Источник.ссылка.Статус = Перечисления._СтатусыЗаявки.ВРаботе или 
            Источник.ссылка.Статус = Перечисления._СтатусыЗаявки.Принят или 
            Источник.ссылка.Статус = Перечисления._СтатусыЗаявки.ПереданоВТестирование)  тогда
