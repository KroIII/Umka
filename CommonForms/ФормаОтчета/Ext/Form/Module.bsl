...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидимостьКомандВариантовОтчетов = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ВидимостьКомандВариантовОтчетов",Истина);	
...
&НаКлиенте
Процедура Сохранить_в_ПДФ(Команда)
    
    КН = Отчет.КомпоновщикНастроек;
    Настр = КН.Настройки;
    ПользНастр = КН.ПользовательскиеНастройки;
    Попытка 
        Каталог = "";
        мКаталог(Каталог) ;
        Режим = РежимДиалогаВыбораФайла.Сохранение; 
        ДиалогСохраненияФайла = Новый ДиалогВыбораФайла(Режим); 
        Если Настр.параметрыДанных.Элементы.Найти("Контрагент")<> Неопределено и Настр.параметрыДанных.Элементы.Найти("Период")<> Неопределено тогда
            ДиалогСохраненияФайла.ПолноеИмяФайла = "Расшифровка_"+ПользНастр.Элементы.Найти(Настр.параметрыДанных.Элементы.Найти("Контрагент").ИдентификаторПользовательскойНастройки).Значение+"_"+Формат(ПользНастр.Элементы.Найти(Настр.параметрыДанных.Элементы.Найти("Период").ИдентификаторПользовательскойНастройки).Значение.ДатаОкончания,"ДФ=MMMMyyyy"); 
           ДиалогСохраненияФайла.ПолноеИмяФайла = СтрЗаменить(ДиалогСохраненияФайла.ПолноеИмяФайла, " ","_"); 
        КонецЕсли;
        Фильтр = "(*.pdf)|*.pdf";
        ...
        Если ДиалогСохраненияФайла.Выбрать() Тогда 
            ...
            ОтчетТабличныйДокумент.Записать(ПутьКФайлу,ТипФайлаТабличногоДокумента.PDF);
        КонецЕсли;  
    Исключение
    КонецПопытки;
...
&НаСервере
Процедура мКаталог(Каталог)
           Каталог =  Константы._КаталогСохраненияОтчетов.Получить();
КонецПроцедуры
