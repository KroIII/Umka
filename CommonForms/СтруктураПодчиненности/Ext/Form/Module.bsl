...
Процедура ВывестиПредставлениеИКартинку(СтрокаДерева,Макет, ЭтоТекущийОбъект = Ложь, ЭтоПодчиненный = Неопределено)
	...
	Иначе
		Если ЭтоТекущийОбъект Тогда
			Если ДеревоПодчиненныеОбъекты.ПолучитьЭлементы().Количество() И ДеревоРодительскиеОбъекты.ПолучитьЭлементы().Количество()  Тогда
				ИмяОбласти = ?( ЭтоДокумент, ?(МетаданныеОбъекта.проведение = Метаданные.СвойстваОбъектов.Проведение.запретить,"ДокументПроведенКоннекторВерхНиз", "ДокументЗаписанКоннекторВерхНиз"), "СправочникПВХКоннекторВерхНиз");
				ОбластьКартинка = Макет.ПолучитьОбласть(ИмяОбласти);
			ИначеЕсли ДеревоПодчиненныеОбъекты.ПолучитьЭлементы().Количество() Тогда
				ИмяОбласти = ?( ЭтоДокумент, ?(МетаданныеОбъекта.проведение = Метаданные.СвойстваОбъектов.Проведение.запретить,"ДокументПроведенКоннекторНиз","ДокументЗаписанКоннекторНиз"), "СправочникПВХКоннекторНиз");
				ОбластьКартинка = Макет.ПолучитьОбласть(ИмяОбласти);
			Иначе
				ИмяОбласти = ?( ЭтоДокумент,  ?(МетаданныеОбъекта.проведение = Метаданные.СвойстваОбъектов.Проведение.запретить,"ДокументПроведенКоннекторВерх","ДокументЗаписанКоннекторВерх"), "СправочникПВХКоннекторВерх");
				ОбластьКартинка = Макет.ПолучитьОбласть(ИмяОбласти);
			КонецЕсли;
		ИначеЕсли ЭтоПодчиненный = Истина Тогда
			Если СтрокаДерева.ПолучитьЭлементы().Количество() Тогда
				ИмяОбласти = ?( ЭтоДокумент, ?(МетаданныеОбъекта.проведение = Метаданные.СвойстваОбъектов.Проведение.запретить,"ДокументПроведенКоннекторЛевоНиз","ДокументЗаписанКоннекторЛевоНиз"), "СправочникПВХКоннекторЛевоНиз");
				ОбластьКартинка = Макет.ПолучитьОбласть(ИмяОбласти);
			Иначе
				ИмяОбласти = ?( ЭтоДокумент, ?(МетаданныеОбъекта.проведение = Метаданные.СвойстваОбъектов.Проведение.запретить,"ДокументПроведен","ДокументЗаписан"), "СправочникПВХКоннекторЛево");
				ОбластьКартинка = Макет.ПолучитьОбласть(ИмяОбласти);
			КонецЕсли;
		Иначе
			Если СтрокаДерева.ПолучитьЭлементы().Количество() Тогда
				ИмяОбласти = ?( ЭтоДокумент, ?(МетаданныеОбъекта.проведение = Метаданные.СвойстваОбъектов.Проведение.запретить,"ДокументПроведенКоннекторЛевоВерх","ДокументЗаписанКоннекторЛевоВерх"), "СправочникПВХКоннекторЛевоВерх");
				ОбластьКартинка = Макет.ПолучитьОбласть(ИмяОбласти);
			Иначе
				ИмяОбласти = ?( ЭтоДокумент, ?(МетаданныеОбъекта.проведение = Метаданные.СвойстваОбъектов.Проведение.запретить,"ДокументПроведен","ДокументЗаписан"), "СправочникПВХКоннекторЛево");
				ОбластьКартинка = Макет.ПолучитьОбласть(ИмяОбласти);
...
Процедура ВывестиРодительскиеОбъекты(ТекущийОбъект, ДеревоРодитель)
	...
	Для Каждого Реквизит Из МетаданныеОбъекта.Реквизиты Цикл
		...
		Если Не Метаданные.КритерииОтбора._СвязанныеЗаявки.Состав.Содержит(Реквизит) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ОбъектыПоКритериюОтбора(ЗначениеКритерияОтбора)
	...
	Если Не Метаданные.КритерииОтбора._СвязанныеЗаявки.Тип.СодержитТип(ТипЗнч(ЗначениеКритерияОтбора))  Тогда
	...
	Запрос.Текст = "ВЫБРАТЬ
                   ...
                   |ИЗ
                   |    КритерийОтбора._СвязанныеЗаявки(&ЗначениеКритерияОтбора) КАК _СвязанныеЗаявки";
...
Функция ПредставлениеОбъектаДляВывода(Данные) 
    ...
    Исключение;
    КонецПопытки;
    Возврат ""+ Формат(Номер, "ЧДЦ=0; ЧГ=0") + " от " + Формат(Данные.ссылка.Дата,"ДФ=d.M.yy")+ " - (" + Данные.ссылка.Контрагент +"):   "+ Данные.ссылка.Название 
    ;
